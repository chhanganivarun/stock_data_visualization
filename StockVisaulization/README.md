...
You should implement your chart in the ``update`` method of the ``indexOfSPChart.js`` file. Within the `IndexOfSPChart` object, you have access to the data through the variable `self.indexOfSnP500Data`.
...
Initialize the chart in the `firstStockPriceChart.js` file in the ``init`` method, modeled after the `init` method in ``indexOfSPChart.js``.
