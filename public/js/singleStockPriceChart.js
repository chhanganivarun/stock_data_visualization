...
SingleStockPricesChart.prototype.init = function()
{
    ...
    // Left div
    var divelectoralVotes = d3.select("#two-stocks").classed("contentL", true);
    self.svgBoundsL = divelectoralVotes.node().getBoundingClientRect();
    self.svgWidthL = self.svgBoundsL.width - self.margin.left - self.margin.right;
    self.svgL = divelectoralVotes.select('#stock-prices');
        ...
        d3.text('./data/daily/table_'+ticker0+'.csv', function(error0, text0) {
            ...
            d3.text('./data/daily/table_'+ticker1+'.csv', function(error1, text1) {
                ...
                if (chartType === 'lines') {
                    ...
                    var valueLineInit = self.valueLineGen(x,
                        d3.scaleLinear()
